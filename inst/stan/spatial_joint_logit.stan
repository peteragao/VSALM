...
transformed parameters {
  ...
  prec_c = 1 / pow(sigma_u, 2);
  theta = mu + sigma_u * u;
  if (K > 0) {
    theta = theta + X * betas;
  }
  for (i in 1:N_data) {
    sigma_e[i] = sqrt(exp(g0 + g1 * log(inv_logit(theta[ind_data[i]]) * (1 - inv_logit(theta[ind_data[i]]))) + g2 * log(df[i]) + sigma_tau * tau[i]));
    V[i] = pow(sigma_e[i] * yhat[i] * (1 - yhat[i]), 2);
...
model {
  ...
  target += pcprec_lpdf(prec_c | 1, .01);
  target += pcprec_lpdf(1 / pow(sigma_tau, 2) | .1, .01);
  ...
  for (i in 1:N_data) {
    ...
    target += normal_lpdf(logit_y[i] | theta[theta_k], sigma_e[i]);
  }
}
