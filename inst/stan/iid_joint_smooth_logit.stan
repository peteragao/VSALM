...
// FIXED EFFECTS
  ...
  real<lower=0> prec_u; // overall precision
...
transformed parameters {
  ...
  for (i in 1:N_data) {
    ...
    sigma_e[i] = sqrt(exp(g0 + g1 * log(inv_logit(theta_obs[ind_data[i]]) * (1 - inv_logit(theta_obs[ind_data[i]]))) + g2 * log(na[i]) + sigma_tau * tau[i]));
...
model {
  ...
  for (i in 1:N_data) {
    ...
    target += normal_lpdf(logit_y[i] | theta_obs[theta_k], sigma_e[i]);
  }
}
generated quantities {
  vector[N] theta;
  for (i in 1:N) {
    theta[i] = normal_rng(theta_obs[i], sigma_u);
  }
  for (i in ind_data) {
    theta[i] = theta_obs[i];
  }
